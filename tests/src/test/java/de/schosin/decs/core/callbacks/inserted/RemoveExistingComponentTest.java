...
public class RemoveExistingComponentTest {

    @Test
    void testRemoveExistingComponent() {
        var world = (InternalWorld) World.builder()
                .add(RemoveExistingComponentTestSystem.class)
                .build();

        var system = world.getSystem(RemoveExistingComponentTestSystem.class);
    ...
    public static abstract class RemoveExistingComponentTestSystem {
        ...
        @EntityInitializer
        final void createEntity(int index, Position pos, Velocity velocity) {
            order.add("createEntity");
        }

        @Transmute(remove = Velocity.class)
        abstract void removeVelocity(Entity entity);

        @EntityInitializer
        final void removeVelocity(int entityId) {
            order.add("removeVelocity");
        }
